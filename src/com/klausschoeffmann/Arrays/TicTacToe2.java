...
public class TicTacToe2 {
    ...
    public static void main(String[] args) {
        short[][][] turns = new short[9][3][3];
        short[][] field = new short[3][3];
        Scanner scanner = new Scanner(System.in);

        while (true) {
            if (playerTurn(scanner, field, turns, 1))
                break;
            if (playerTurn(scanner, field, turns, 2))
                break;
        }

        System.out.println("All game situations:");
        printTurns(turns);
    }

    private static boolean playerTurn(Scanner scanner, short[][] field, short[][][] turns, int playerNumber) {
        ...
        //save current game situation
        for (int y = 0; y < field.length; y++) {
            for (int x = 0; x < field[y].length; x++) {
                turns[turnCount][y][x] = field[y][x];
            }
        }
    ...
    private static void printTurns(short[][][] f) {
        for (short[][] field : f) {
            printField(field);
            System.out.println("########");
        }
